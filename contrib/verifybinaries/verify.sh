...
###   vivuscoin.com and vivuscoin.org and compares them.
...
WORKINGDIR="/tmp/vivuscoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://vivuscoin.com"
HOST2="https://vivuscoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="vivuscoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'vivuscoin-',
...
if [ $? -ne 0 ]; then
   echo "vivuscoin.org failed to provide signature file, but vivuscoin.com did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "vivuscoin.org and vivuscoin.com signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Vivuscoin Core binary release signing key installed?"
   fi
