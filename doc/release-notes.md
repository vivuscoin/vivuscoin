Vivuscoin Core version 0.18.0 is now available from:

  <https://vivuscoin.com/bin/vivuscoin-core-0.18.0/>
...
`/Applications/Vivuscoin-Qt` (on Mac) or `vivuscoind`/`vivuscoin-qt` (on
Linux).
...
Vivuscoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Vivuscoin Core on unsupported systems.
...
10.10. Additionally, Vivuscoin Core does not yet change appearance when
...
`contrib/init/vivuscoind.service`, it has been changed to use
`/var/lib/vivuscoind` as the data directory instead of
`~vivuscoin/.vivuscoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/vivuscoind` will
exist has enough space (check using `df -h /var/lib/vivuscoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/vivuscoin/vivuscoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://vivuscoin.com/en/contact/) the Vivuscoin Core
...
The systemd init file (`contrib/init/vivuscoind.service`) has been changed
to use `/var/lib/vivuscoind` as the data directory instead of
`~vivuscoin/.vivuscoin`. This change makes Vivuscoin Core more consistent with
...
`datadir` option in `/etc/vivuscoin/vivuscoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/vivuscoin/vivuscoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/vivuscoin/vivuscoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/vivuscoin/vivuscoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/vivuscoin/vivuscoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/vivuscoin/vivuscoin/blob/master/doc/vivuscoin-conf.md)
  about the `vivuscoin.conf` file describes how to use it to configure
  Vivuscoin Core.

- A new document introduces Vivuscoin Core's BIP174 [Partially-Signed
  Vivuscoin Transactions
  (PSBT)](https://github.com/vivuscoin/vivuscoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/vivuscoin/vivuscoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Vivuscoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Vivuscoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/vivuscoin/vivuscoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/vivuscoin/vivuscoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Vivuscoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Vivuscoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/vivuscoin/vivuscoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/vivuscoin/vivuscoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart vivuscoind with the `-deprecatedrpc=generate`
...
- A new `vivuscoin-wallet` tool is now distributed alongside Vivuscoin
...
This section describes planned changes to Vivuscoin Core that may affect
other Vivuscoin software and services.

- Since version 0.16.0, Vivuscoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Vivuscoin Core 0.20 (expected about
  a year after 0.18), Vivuscoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Vivuscoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Vivuscoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Vivuscoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Vivuscoin Core
...
- A sub-project of Vivuscoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Vivuscoin Core.  See their [project
  page](https://github.com/vivuscoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Vivuscoin Core's own implementation, although entropy
  gathered by Vivuscoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Vivuscoin Core a
...
- On macOS, Vivuscoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `vivuscoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libvivuscoin_server.a` (jnewbery)
...
- #13926 [Tools] vivuscoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like vivuscoind (MarcoFalke)
...
- #14521 qt, docs: Fix `vivuscoin-qt -version` output formatting (hebasto)
...
- #11625 Add VivuscoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libvivuscoin_server` (laanwj)
...
- #15154 Configure: vivuscoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/vivuscoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Vivuscoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libvivuscoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in vivuscoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Vivuscoin Core (Christewart)
...
- #12246 Bugfix: Only run vivuscoin-tx tests when vivuscoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_vivuscoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_vivuscoin` (MarcoFalke)
...
- #14186 vivuscoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_vivuscoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update vivuscoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/vivuscoin-conf.md (hebasto)
...
- #15354 Add missing `vivuscoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct vivuscoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support vivuscoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/vivuscoin/).
