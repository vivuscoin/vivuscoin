...
const char * const VIVUSCOIN_CONF_FILENAME = "vivuscoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "vivuscoind -foo=bar
...
#else
    const char* pszModule = "vivuscoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Vivuscoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Vivuscoin
    // Mac: ~/Library/Application Support/Vivuscoin
    // Unix: ~/.vivuscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Vivuscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Vivuscoin";
#else
    // Unix
    return pathRet / ".vivuscoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", VIVUSCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Vivuscoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Vivuscoin Core") == std::string::npos) {
