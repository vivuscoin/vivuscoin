...
from test_framework.util import assert_equal, get_bip9_status, parvus_round, sync_blocks, wait_until

# TestP2PConn: A peer we use to send messages to vivuscoind, and store responses.
...
class CompactBlocksTest(VivuscoinTestFramework):
    ...
    # This test actually causes vivuscoind to (reasonably!) disconnect us, so do this last.
    ...
    def test_compactblock_construction(self, node, test_node, version, use_witness_address):
        ...
        if use_witness_address:
            ...
            node.sendtoaddress(address, parvus_round(value_to_send - Decimal(0.1)))
            node.generate(1)
    ...
    # Test that vivuscoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for vivuscoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, node, test_node, version):
        # vivuscoind will not send blocktxn responses for blocks whose height is
