...
from test_framework.util import assert_equal, assert_raises_rpc_error, parvus_round, sync_blocks, sync_mempools

MAX_ANCESTORS = 25
MAX_DESCENDANTS = 25

class MempoolPackagesTest(VivuscoinTestFramework):
    ...
    def chain_transaction(self, node, parent_txid, vout, value, fee, num_outputs):
        send_value = parvus_round((value - fee)/num_outputs)
    ...
    def run_test(self):
        ...
        for x in reversed(chain):
            descendant_fees += mempool[x]['fee']
            if (x == chain[-1]):
                assert_equal(mempool[x]['modifiedfee'], mempool[x]['fee']+parvus_round(0.00002))
                assert_equal(mempool[x]['fees']['modified'], mempool[x]['fee']+parvus_round(0.00002))
            assert_equal(mempool[x]['descendantfees'], descendant_fees * COIN + 2000)
            assert_equal(mempool[x]['fees']['descendant'], descendant_fees+parvus_round(0.00002))
        ...
        send_value = parvus_round((value - fee)/2)
