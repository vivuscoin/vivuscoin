...
class FullBlockTest(VivuscoinTestFramework):
    ...
    def run_test(self):
        ...
        # Extend the b26 chain to make sure vivuscoind isn't accepting b26
        ...
        # Extend the b28 chain to make sure vivuscoind isn't accepting b28
        ...
        # Create a transaction that spends one parvus to the p2sh_script, the rest to OP_TRUE
        ...
        # Until block is full, add tx's with 1 parvus to p2sh_script, the rest to OP_TRUE
        tx_new = None
        tx_last = tx
        ...
        # vivuscoind doesn't disconnect us for sending a bloated block, but if we subsequently
        ...
        #                                                                                    \-> b68 (20)
        #
        # b68 - coinbase with an extra 10 parvuses,
        #       creates a tx that has 9 parvuses from out[20] go to fees
        #       this fails because the coinbase is trying to claim 1 parvus too much in fees
        #
        # b69 - coinbase with extra 10 parvuses, and a tx that gives a 10 parvus fee
        #       this succeeds
        #
        ...
        #    Low-S and High-S values (whereas the vivuscoin code has custom code which does so);
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), solve=True, *, version=1):
        ...
        if spend is None:
            ...
            coinbase.vout[0].nValue += spend.vout[0].nValue - 1  # all but one parvus to fees
            coinbase.rehash()
            block = create_block(base_block_hash, coinbase, block_time, version=version)
            tx = self.create_tx(spend, 0, 1, script)  # spend 1 parvus
            self.sign_tx(tx, spend)
