...
from test_framework.util import assert_equal, assert_raises_rpc_error, bytes_to_hex_str, parvus_round
...
def make_utxo(node, amount, confirmed=True, scriptPubKey=CScript([1])):
    ...
    """
    fee = 1*COIN
    while node.getbalance() < parvus_round((amount + fee)/COIN):
        node.generate(100)

    new_addr = node.getnewaddress()
    txid = node.sendtoaddress(new_addr, parvus_round((amount+fee)/COIN))
...
class ReplaceByFeeTest(VivuscoinTestFramework):
    ...
    def test_doublespend_chain(self):
        ...
        # child fees - 40 VVC - so this attempt is rejected.
        dbl_tx = CTransaction()
